...
<?php 
	...
	if (isset($_POST['work_pending_down']))
	{
		$date1=$_POST['date3'];
		$date2=$_POST['date4'];
		...
		{
			$query="SELECT * FROM cust_hfiber_workorder WHERE hfiber_cust_team='Field Support' AND hfiber_cust_status != 'RESOLVED' AND hfiber_creation_date BETWEEN '$date1' AND '$date2' ORDER BY hfiber_id DESC";
		}
		elseif(custSession::get('user_role')=='RF')
		{
			$query="SELECT * FROM cust_hfiber_workorder WHERE hfiber_cust_team='Optical Team' AND hfiber_cust_status != 'RESOLVED' AND hfiber_creation_date BETWEEN '$date1' AND '$date2' ORDER BY hfiber_id DESC";
		}
		else
		{
			$query="SELECT * FROM cust_hfiber_workorder WHERE hfiber_cust_status != 'RESOLVED' AND hfiber_creation_date BETWEEN '$date1' AND '$date2' ORDER BY hfiber_id DESC";
		}
		...
		if ($result->num_rows>=0)
		{
			...
			';
				...
				header("Content-Disposition:attachment; filename=HFIBER_PENDING_WORK_ORDER.xls");
